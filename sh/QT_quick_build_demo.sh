...
UIdir=$(find application/reference_ui2 -type d -name $refer1)
UIdir1=$(find application/reference_ui2 -type d -name $refer1|sed "s/$refer1/$target1/")
isUIdir=$(find application/reference_ui2 -type d -name $refer1| wc -l)
if [ $isUIdir -ge 1  ]     # -ge大于等于  -le 小于等于 
then
echo "refer exist in UIdir"
#cp -r $UIdir $UIdir1

find application/reference_ui2 -type d -name "$refer1"|sed "s/$refer1//"|xargs -i cp -r {}/$refer1 {}/$target1
find application/reference_ui2 -type d -name "$target1" >> filepath.txt
...
sed -i "s/defined($refer1)/defined($refer1)||defined($target1)/g" `grep "defined($refer1)" -rl application/reference_ui2`

sed -i "s/defined\s*(\s*$refer1\s*)/defined($refer1)||defined($target1)/g" `grep "defined\s*(\s*$refer1\s*)" -rl application/reference_ui2`
